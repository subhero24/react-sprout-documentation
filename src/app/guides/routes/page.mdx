...
The url `/app/admin/posts` for example has three segments; `app`, `admin` and `posts` which will respectively match the paths of the `Root`, `Admin`, and `Posts` routes.
...
<Info>
	If the `Admin` route does not render anything but `props.children`, it is not necessary to group `Users` and `Posts`.
